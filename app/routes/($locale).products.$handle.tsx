...
export default function Product() {
  ...
  const { sizeOptionName, sizes, selectedSize } = useMemo(() => {
    const sizeOption =
      productOptions.find((opt) => opt.name.toLowerCase() === 'size') ||
      productOptions.find((opt) => opt.name.toLowerCase().includes('size'));
    const optionName = sizeOption?.name ?? null;
    const values = sizeOption?.optionValues?.map((v) => v.name) ?? [];
    const selected =
      optionName && selectedVariant?.selectedOptions
        ? selectedVariant.selectedOptions.find((o) => o.name === optionName)?.value ?? null
        : null;

    return { sizeOptionName: optionName, sizes: values, selectedSize: selected };
  }, [productOptions, selectedVariant?.selectedOptions]);
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-900">
      ...
      {expandedImage && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8 bg-black/90 backdrop-blur-md transition-opacity duration-300"
          onClick={() => setExpandedImage(null)}
        >
          <div className="relative max-w-5xl w-full max-h-[95vh] flex flex-col items-center justify-center gap-6" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => setExpandedImage(null)}
            ...
            <div className="bg-stone-900/80 backdrop-blur-md border border-white/10 rounded-2xl p-5 w-full max-w-md shadow-2xl flex flex-col gap-3">
              ...
              <p className="text-stone-300 text-sm italic font-medium">"{expandedImage.review}"</p>
            </div>
          </div>
        </div>
      )}
            ...
            {displayTags.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-6">
                {displayTags.map((t) => (
                  <span
                    key={t}
                    className="inline-flex items-center px-3 py-1 rounded-full bg-white border border-stone-200 text-[10px] tracking-widest uppercase font-semibold text-stone-600"
                  >
                    {t}
                  </span>
                ))}
            ...
            {testimonials.length > 0 && (
              <div className="flex items-center gap-3 mb-6">
                <div className="flex gap-1">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} className={`w-3.5 h-3.5 ${i < Math.round(Number(avgRating)) ? 'text-stone-900' : 'text-stone-200'}`} fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292Z" />
                    </svg>
                  ))}
                </div>
                <a href="#reviews" className="text-xs text-stone-500 tracking-wider uppercase font-medium hover:text-stone-900 transition-colors">
                ...
                <div className="flex flex-wrap gap-2">
                  {sizes.map((s) => {
                    ...
                    return (
                      <span
                        key={s}
                        className={`inline-flex items-center px-4 py-2 rounded-full text-[10px] font-bold tracking-widest uppercase border ${isSelected
              ...
              {testimonials.length > 0 ? (
                <div className="space-y-6">
                  {testimonials.map((review: any, idx: number) => (
                    <div key={idx} className="bg-white border border-stone-200 rounded-2xl p-6 sm:p-8 shadow-sm flex flex-col gap-6">
                      ...
                      <div className="flex items-start sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          {review.customer_image ? (
                            ...
                            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-stone-50 border border-stone-100 flex items-center justify-center shrink-0 shadow-inner">
                          ...
                          <div className="flex flex-col gap-1">
                            <p className="text-sm font-bold tracking-widest text-stone-900 uppercase">{review.name}</p>
                      ...
                      {/* Review Text Body */}
                      <p className="text-[15px] sm:text-base text-stone-700 leading-relaxed font-medium">
                        "{review.review}"
                      </p>
                      ...
                      {review.customer_product_image && (
                        <div
                          className="mt-2 w-28 h-28 sm:w-36 sm:h-36 group relative overflow-hidden rounded-xl border border-stone-200/60 shadow-md cursor-pointer bg-stone-50 shrink-0"
                        ...
                        >
                          ...
                          <div className="absolute inset-0 bg-transparent group-hover:bg-black/10 transition-colors duration-300 pointer-events-none flex items-center justify-center">
                            <div className="bg-white/90 backdrop-blur-md text-stone-900 rounded-full p-2.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-2 group-hover:translate-y-0 shadow-lg border border-stone-100">
