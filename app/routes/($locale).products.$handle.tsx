      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-24">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 lg:items-start">
          ...
          <div className="lg:sticky lg:top-10">
            ...
            {/* Main Image */}
            <div className="aspect-square overflow-hidden rounded-3xl bg-white dark:bg-card border border-stone-200/60 dark:border-border shadow-[0_4px_20px_-4px_rgba(0,0,0,0.08)] mb-4 relative group">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-3xl" />
              {/* Image counter badge */}
              {images.length > 1 && (
                <div className="absolute bottom-3 right-3 bg-white/90 dark:bg-card/90 backdrop-blur-md text-stone-700 dark:text-foreground px-3 py-1.5 rounded-full text-[10px] font-bold tracking-wider shadow-lg border border-stone-100 dark:border-border">
              ...
              <div className="flex gap-2.5 overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                {images.map((img, idx) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-xl overflow-hidden border-2 transition-all duration-300 cursor-pointer hover:scale-105 ${
                      idx === selectedImageIndex
                        ? 'border-stone-900 dark:border-foreground shadow-md ring-2 ring-stone-900/10 dark:ring-foreground/10 opacity-100'
                        : 'border-transparent opacity-50 hover:opacity-100 hover:border-stone-300 dark:hover:border-border'
                    }`}
                  >
                    <Image
                      data={img}
          ...
          <div className="lg:py-2 flex flex-col">
            ...
            <div className="mb-8 relative pl-6">
              <div className="absolute left-0 top-0 bottom-0 w-[3px] rounded-full bg-gradient-to-b from-amber-400/80 via-amber-300/40 to-transparent" />
            ...
            <div className="relative my-10">
              <div className="h-px bg-gradient-to-r from-transparent via-stone-300 dark:via-border to-transparent" />
              ...
              {[
                ...
                <details key={section.id} open={section.defaultOpen} className="group border-b border-stone-200/60 dark:border-border/60">
                  <summary className="flex items-center justify-between cursor-pointer py-5 sm:py-6 select-none list-none outline-none hover:bg-stone-50/50 dark:hover:bg-muted/20 -mx-2 px-2 rounded-lg transition-colors duration-200">
                    ...
                    {/* Plus/Minus icon */}
                    <div className="w-6 h-6 rounded-full border border-stone-300 dark:border-border flex items-center justify-center flex-shrink-0 group-open:bg-stone-900 group-open:border-stone-900 dark:group-open:bg-foreground dark:group-open:border-foreground transition-all duration-300">
                      <svg
                  ...
                  <div className="flex items-center gap-2.5 bg-stone-50 dark:bg-muted/30 border border-stone-200/60 dark:border-border rounded-xl px-4 py-2.5">
                    ...
                    <div className="flex flex-col gap-0.5">
                      <div className="flex gap-0.5">
                        {[...Array(5)].map((_, i) => (
                          <svg key={i} className={`w-3 h-3 ${i < Math.round(Number(avgRating)) ? 'text-amber-400' : 'text-stone-200 dark:text-border'}`} fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292Z" />
                          </svg>
                        ))}
                      </div>
              ...
              {testimonials.length > 0 ? (
                <div className="space-y-5">
                  {testimonials.map((review: any, idx: number) => (
                    <div key={idx} className="relative bg-white dark:bg-card border border-stone-200/60 dark:border-border rounded-2xl p-6 sm:p-8 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.06)] hover:shadow-md transition-shadow duration-300 flex flex-col gap-5 overflow-hidden">
            ...
            <div className="mt-14">
              ...
              <div className="relative">
                ...
                <div className="absolute left-[19px] top-8 bottom-8 w-px bg-gradient-to-b from-amber-300/60 via-amber-200/40 to-transparent dark:from-amber-700/40 dark:via-amber-800/20" />
    ...
    <section className="mt-20 md:mt-32 pt-16 relative">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        {displayProducts.map((product: any, index: number) => (
          <div
            key={product.id}
            className="group bg-white dark:bg-card border border-stone-200/60 dark:border-border rounded-2xl overflow-hidden shadow-[0_2px_12px_-4px_rgba(0,0,0,0.06)] hover:shadow-lg transition-all duration-300 hover:-translate-y-1 flex flex-col"
          >
            <Link
            ...
            >
              ...
              {product.tags?.[0] && (
                <div className="absolute top-2.5 right-2.5">
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-white/90 dark:bg-background/90 backdrop-blur-sm border border-stone-200/60 dark:border-border text-foreground text-[9px] font-bold tracking-wider uppercase rounded-full shadow-sm">
                    ✓ Certified
                  </span>
                </div>
              )}
